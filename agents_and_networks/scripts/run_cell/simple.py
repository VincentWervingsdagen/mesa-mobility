...
def main(model_params):
    ...
    output_writer.writerow(['id','owner','device','timestamp','cellinfo.wgs84.lon','cellinfo.wgs84.lat','cellinfo.azimuth_degrees','cellinfo.frequency','cellinfo.height','cellinfo.power','cellinfo.postal_code'])
        ...
        for phone in range(1):
            ...
            while(p_time <= max):
                ...
                if (x_old != x or y_old != y):
                    ...
                    postal_code = df_cell['Postal_code'].iloc[index_dis]

                agent = re.sub("[^0-9]", "", agents[i])

                output_writer.writerow([writing_id, f"Agent{agent}", f"{agent}_{phone+1}",
                    start + timedelta(seconds = p_time), cellx, celly, degree,frequency,height,power,postal_code])
